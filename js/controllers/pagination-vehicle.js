...
function buildTable() {
	var table = $('#table-vehicle');

	var data = pagination(state.querySet, state.page, state.rows);
	var myList = data.querySet;

	for (var i = 0; i < myList.length; i++) {
		if (myList[i].type == "truck")
			var row1 = `<tr> <td class="pt-4"><i class="fa-solid fa-truck"></i> ${myList[i].id}</td>`;
		else
			var row1 = `<tr> <td class="pt-4"><i class="fa-solid fa-archive"></i> ${myList[i].id}</td>`;
